...
namespace BrnMall.Web.Controllers
{
    ...
    public partial class OrderController : BaseWebController
    {
        ...
        public ActionResult ConfirmOrder()
        {
            ...
            var config = StoreConfigManager.GetStoreCreditConfigInfo(WorkContext.StoreId);
            model.PayCreditName = config?.PayCreditName??"";
            model.UserPayCredits = WorkContext.PartUserInfo.PayCredits;
            model.MaxUsePayCredits = Credits.GetOrderMaxUsePayCredits(WorkContext.StoreId, WorkContext.PartUserInfo.PayCredits);
            ...
            lock (_locker)
            {
                ...
                if (payCreditCount > 0)
                {
                    ...
                    if (payCreditCount > WorkContext.PartUserInfo.PayCredits)
                        return AjaxResult("noenoughpaycredit", "你使用的" + (config?.PayCreditName??"") + "数超过你所拥有的" + WorkContext.PartUserInfo.PayCredits + "数");
                    if (payCreditCount > (config?.OrderMaxUsePayCredits ?? 0) * selectedStoreCount)
                        return AjaxResult("maxusepaycredit", "此笔订单最多使用" + (config?.OrderMaxUsePayCredits??0) * selectedStoreCount + "个" + (config?.PayCreditName??""));
                }
